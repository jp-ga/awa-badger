...
def fit_gaussian_linear_background(y, inital_guess=None, visualize=True, n_restarts=1):
    ...
    # create LSQ model
    model = GaussianLeastSquares(torch.tensor(x), torch.tensor(normed_y), torch.tensor(pk_loc))
    smoothed_model = GaussianLeastSquares(torch.tensor(x), torch.tensor(smoothed_y), torch.tensor(pk_loc))
